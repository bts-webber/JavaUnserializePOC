...
class CheckVulnerability(object):
    ...
    def CheckJboss(self,target):
        ...
        host=com_ip.match(target)
        print com_port.match(target)[1:]
        port=int(com_port.match(target)[1:])
        server=(host,port)
        ...
        payload_3=payload_3.replace(":","").decode("hex")
        try:
            self.SendPayload(server,payload_1)
            self.SendPayload(server,payload_2)
            data=self.SendPayload(server,payload_3)
        except:
